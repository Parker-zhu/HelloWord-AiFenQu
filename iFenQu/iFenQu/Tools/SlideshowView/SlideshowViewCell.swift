...
class SlideshowViewCell: UICollectionViewCell {
    // 标题
    var title: String = "" {
        didSet {
    ...
    override func layoutSubviews() {
        ...
        titleBackView.frame = CGRect.init(x: 0, y: self.height - titleLabelHeight, width: self.width, height: titleLabelHeight)
        titleLabel.frame = CGRect.init(x: titleLabelLeading, y: 0, width: self.width - titleLabelLeading - 5, height: titleLabelHeight)
    }
}
